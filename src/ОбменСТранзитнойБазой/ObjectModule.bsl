...
Процедура СоздатьДокументы1С(ИдентификаторПакета, ОбновитьДокумент = 0)
	...
	Попытка
		Результат = ВыполнитьКоманду(ОбщиеФункции._СтрШаблон_("exec GetDeliveryResponce @DocNum='%1', @Update=%2", ИдентификаторПакета, ОбновитьДокумент));
	Исключение
		ВызватьИсключение ОписаниеОшибки();
	КонецПопытки;
