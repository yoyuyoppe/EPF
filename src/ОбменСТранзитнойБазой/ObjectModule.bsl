...
#Область УПРАВЛЕНИЕ_СКЛАДОМ
	...
	Процедура СоздатьИнвОписи(ТабЗнач, АдресСпискаДокументов, АдресТаблицыДляОбработкиОшибок)
		...
		Для каждого СтрТехноЗона Из ТаблицаСТехноЗонами Цикл
			...
			ДокОбъект.Дата = ТекущаяДата(); 
			ДокОбъект.Склад = ПодобратьСкладДляВладельцаЗапасов(ПолучитьВладельцаЗапасов(СтрТехноЗона.OwnerStock));
...
#Область ПРОЧЕЕ
	...
	Функция ПреобразоватьДанныеВМС_В_1С(Состав)
		...
		Для каждого ЭлКоллекции Из Состав Цикл
			...
			ИначеЕсли (СтруктураЗаписи.Номенклатура.МодельСкладскогоУчета.КонтрольДатыПроизводства
				...
				СтруктураЗаписи.ДатаПроизводства	= Дата(1,1,1);				
	...
	Процедура Инициализация()
		...
		ЛогКомплекс_СОХ = Справочники.Склады.ПустаяСсылка(); // ЛОГИСТИЧЕСКИЙ КОМПЛЕКС (СОХ), пока отсутствует
