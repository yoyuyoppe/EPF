...
Функция ПолучитьТаблицуАналоговОсновныхКарточек(Склад)
	...
	|	влЗапрос.Номенклатура.ЕдиницаХраненияОстатков КАК ЕдиницаИзмерения,
	|	Остатки.СерияНоменклатуры,
	|	Остатки.СерияНоменклатуры.ДатаПроизводства КАК ДатаПроизводства,
	|	Остатки.Качество, 
...
Процедура РаспределитьКоличествоОсновнойКарточкиПоАналогам(Состав, Склад)
	...
	Для Сч=0 По КоличествоСтрок-1 Цикл
		...
		Аналоги = ТаблицаАналоговОсновныхКарточек.НайтиСтроки(Новый Структура("ОсновнаяКарточкаНоменклатуры, ДатаПроизводства, Качество", СтрТаб.Номенклатура, ДатаПроизводства, СтрТаб.Качество)); 
		...
		Для каждого Аналог Из Аналоги Цикл
			...
			Если СтрТаб.Количество > Аналог.КоличествоОстаток Тогда
				НовСтр.Количество = Аналог.КоличествоОстаток;
				СтрТаб.Количество = СтрТаб.Количество - Аналог.КоличествоОстаток;
			ИначеЕсли СтрТаб.Количество < Аналог.КоличествоОстаток Тогда 
				Аналог.КоличествоОстаток = Аналог.КоличествоОстаток - СтрТаб.Количество;
				СтрТаб.Количество = 0;
			КонецЕсли; 
			
			Если СтрТаб.Количество <= 0 Тогда
				Лог1С.Вывести(ОбщиеФункции._СтрШаблон_("Товар '%1' полностью распределился на аналог '%2'", СтрТаб.Номенклатура, НовСтр.Номенклатура), СтатусСообщения.Внимание);
