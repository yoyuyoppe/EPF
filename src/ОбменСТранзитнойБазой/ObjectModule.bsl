...
Процедура СоздатьДокументыКорректировкиЗапасов(НачДата = Неопределено, КонДата = Неопределено)
	...
	Попытка
		...
		Если ОтборКорректировокЗаПериод Тогда
			Результат = ВыполнитьКоманду(ОбщиеФункции._СтрШаблон_("select ISNULL(TRY_CONVERT(nvarchar(50),tid),'') AS tid, DocumentCode, CorrectionCode, 
			|CASE 
			|	WHEN DocumentCode = 'SYSTEM'
			|		THEN 'SYSTEM'
			|	ELSE
			|		OperationCode
			|END AS  OperationCode, MaterialCode, Quantity, MaterialUnitCode, QualityTypeCode, OwnerCode, DOCNUM, ProductionDate, ExpirationDate
		...
		Иначе
			Результат = ВыполнитьКоманду("select select ISNULL(TRY_CONVERT(nvarchar(50),tid),'') AS tid, DocumentCode, CorrectionCode,
			|CASE 
			|	WHEN DocumentCode = 'SYSTEM'
			|		THEN 'SYSTEM'
			|	ELSE
