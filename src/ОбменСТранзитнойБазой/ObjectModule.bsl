...
Функция ПолучитьСериюНоменклатурыПоДатеПроизводства(Владелец, ДатаПроизводства, ДопПараметрыДляЕГАИС = Неопределено, ДопПараметрыМеркурий = Неопределено,Создавать=ложь,СрокГодности=неопределено)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗапасыНаСкладах.Остатки(, Номенклатура = &Владелец) КАК ЗапасыНаСкладах
