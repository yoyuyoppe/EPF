...
#Область УПРАВЛЕНИЕ_ЗАПАСАМИ
	...
	Функция СоздатьСписаниеТоваров(ВладелецЗапаса, ТипОперации, Организация, Состав)
		...
		ДокОбъект.Комментарий		= "Создана автоматически на основании данных WMS";
	...
	Процедура СоздатьДокументыКорректировкиЗапасов()
		...
		Попытка
			...
			//Результат = ВыполнитьКоманду("select ISNULL(TRY_CONVERT(nvarchar(50),tid),'') AS tid, DocumentCode, CorrectionCode, OperationCode, MaterialCode, Quantity, MaterialUnitCode, QualityTypeCode, OwnerCode, DOCNUM, ProductionDate, ExpirationDate
			//|from StockCorrection where RecordDate between '08-05-2020' and '09-05-2020' and CHECKED_BY_PI is null"); // для отладки
		Исключение
		...
		Для каждого СтрТипОперации Из тзТипыОпераций Цикл
			...
			Если Найти("SERIES, DATE, OWNER, STOCK", СтрТипОперации.ТипОперации) > 0 Тогда
				Продолжить;
			КонецЕсли;  
			...
			ИначеЕсли СтрТипОперации.ТипОперации = "-" Тогда 
				...
				//	
				//	УстановитьСвязьМеждуПарнымиДвижениями(Новый Структура("Объект, Основание", ДокСписание, ДокОприходование.Ссылка),
				//				Новый Структура("Объект, Основание", ДокОприходование, ДокСписание.Ссылка));
				//	
			...
			Для каждого ЭлМассива Из Состав Цикл
				...
				Если СписокИдПакетовДляПодтверждения.НайтиПоЗначению(ЭлМассива.ИдПакета) = Неопределено Тогда
	...
	Процедура СоздатьДокументыКорректировкиЗапасовПоКнопке() экспорт
...
#Область ПРОЧЕЕ
	...
	Процедура УстановитьСвязьМеждуПарнымиДвижениями(Связь1, Связь2)
		//		
	КонецПроцедуры
...
КонецПроцедуры 
	...
	Процедура ТестПреобразоватьДанныеВМС_В_1С_КорректировкаЗапасов()
	...
	Процедура ТестДобавитьПринадлежностьЗапасовКОрганизации()
	...
	Процедура ТестПреобразоватьДанныеВМС_В_1С_ОтветПоЗаявкам()
...
//ТестПреобразоватьДанныеВМС_В_1С_КорректировкаЗапасов();
//ТестДобавитьПринадлежностьЗапасовКОрганизации();
//ТестПреобразоватьДанныеВМС_В_1С_ОтветПоЗаявкам();
//ТестСоздатьДокументыКорректировкиЗапасов();
