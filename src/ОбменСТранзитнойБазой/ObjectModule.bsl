...
Перем КоличествоДнейДляКонтроляОтменыПроведения;
Перем ПепсикоХолдинг;
Перем ТаблицаСопосталенныхПричинНедоборов;
...
Процедура ВыгрузитьЗаявкиВТранзит()
	...
	Для каждого СтрЗаявка Из Заявки Цикл
		...
		Попытка
			ВыполнитьКоманду(СтрЗаявка.Текст);
		Исключение
...
Процедура СоздатьДокументы1С(ИдентификаторПакета, ОбновитьДокумент = Ложь)
	...
	Попытка
		Результат = ВыполнитьКоманду(ОбщиеФункции._СтрШаблон_("exec GetDeliveryResponce @DocNum='%1', @Update=%2", ИдентификаторПакета, Формат(ОбновитьДокумент, "БЛ=0; БИ=1")));
	Исключение
		ВызватьИсключение ОписаниеОшибки();
	КонецПопытки;
...
Функция ПолучитьДанныеДляЗаполненияДокументов(Ответ)
	...
	Пока НЕ Ответ.EOF Цикл
		...
		Если Ответ.Fields("DeliveryTypeCode").Value <> NULL Тогда
			...
			ДляЗаполнения.Состав.Колонки.Добавить("ПричинаНедобора", Новый ОписаниеТипов("СправочникСсылка.ПричиныВозвратовОтЭкспедиторов"));
		...
		Иначе
			...
			Если Ответ.Fields("ReasonCode").Value <> NULL Тогда
				НовСтр.ПричинаНедобора = ПолучитьПричинуНедобораПоВнешнемуКоду(Ответ.Fields("ReasonCode").Value);
			КонецЕсли; 
	...
	Если ЗначениеЗаполнено(ДляЗаполнения.Ссылка) Тогда
		...
		Для каждого СтрТабЧасть Из ДляЗаполнения.Ссылка.Товары Цикл
			...
			Если ТипДокумента = "IN" Тогда
				Прервать;	
			КонецЕсли; 
			//ToDo: Надо пересчитать количество в ШТ.
			Если ДляЗаполнения.Состав.Найти(СтрТабЧасть.GUID_Строки, "GUID_Строки") = Неопределено Тогда
				...
				НовСтр.Количество = ?(НовСтр.ЕдиницаИзмерения.Коэффициент > НовСтр.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент,
				НовСтр.Количество * НовСтр.ЕдиницаИзмерения.Коэффициент,   
				НовСтр.Количество / НовСтр.Номенклатура.ЕдиницаХраненияОстатков.Коэффициент);
...
Процедура ОбработкаЗаполненияДокументов(ДляЗаполнения)
	...
	ИначеЕсли ТипЗнч(ДляЗаполнения.Ссылка) = Тип("ДокументСсылка.ПеремещениеТоваров")
		И (ДляЗаполнения.Ссылка.Склад = ЛогКомплекс ИЛИ ДляЗаполнения.Ссылка.Склад = ЛогКомплекс_СОХ) Тогда 	// Отправитель лог. комплекс
		
		СоздатьДокументыПоИсходящимПеремещениям(ДляЗаполнения);
		
	ИначеЕсли ТипЗнч(ДляЗаполнения.Ссылка) = Тип("ДокументСсылка.ПеремещениеТоваров")
		И (ДляЗаполнения.Ссылка.СкладПолучатель = ЛогКомплекс ИЛИ ДляЗаполнения.Ссылка.СкладПолучатель = ЛогКомплекс_СОХ) Тогда 	// Получатель лог. комплекс
...
Процедура ОтменитьПроведениеДокумента(ИдДокумента, ТипДокумента, ДатаУдаленияБ2Б, ПользовательВМС)
	...
	// Данная проверка создана для чистки B2B. Если дата меньше установленного среза, 
	// тогда отмена проведения не требуется
	Если ДокСсылка.Дата < ОбщиеФункции.ДобавитьДень(ТекущаяДатаСеанса(), -КоличествоДнейДляКонтроляОтменыПроведения) Тогда
		Возврат;
	КонецЕсли; 
...
Процедура СоздатьОтборЗапасовНаСкладе(ДляЗаполнения)
	...
	УчитыватьСерииВЛистеОтбора = (ДляЗаполнения.Ссылка.Товары.НайтиСтроки(Новый Структура("СерияНоменклатуры", Справочники.СерииНоменклатуры.ПустаяСсылка())).Количество() = 0);
	
	Для каждого вмсСтрТаб Из ДляЗаполнения.Состав Цикл
		
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Номенклатура", вмсСтрТаб.Номенклатура);
		
		Если УчитыватьСерииВЛистеОтбора Тогда
			СтруктураОтбора.Вставить("СерияНоменклатуры", вмсСтрТаб.СерияНоменклатуры);
		КонецЕсли; 
		
		СтрокиЛистаОтбора = ДокОбъект.ЛистОтбора.НайтиСтроки(СтруктураОтбора);
		
		Если ЗначениеЗаполнено(СтрокиЛистаОтбора) Тогда
			НовСтр = СтрокиЛистаОтбора.Получить(0);
		...
		Если ЗначениеЗаполнено(вмсСтрТаб.ДатаПроизводства) Тогда
			...
			НовСтр.СерияНоменклатурыОтбора = ПолучитьСериюНоменклатурыПоДатеПроизводства(НовСтр.Номенклатура, вмсСтрТаб.ДатаПроизводства, ДопПараметрыДляЕГАИС,,,, НовСтр.Качество);
	...
	Если ЗначениеЗаполнено(ДляЗаполнения.Ссылка.ВозвратнаяТара) Тогда
		ПеренестиВозвратнуюТаруИзДокументаОтгрузки(ДокОбъект, ДляЗаполнения.Ссылка);
	КонецЕсли; 
	...
	Попытка		
		ДокОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Оперативный);
	Исключение
		Лог1С.Вывести("Не удалось провести отбор запасов на основании '{0}' по причине: {1}", СтатусСообщения.ОченьВажное, ДляЗаполнения.Ссылка, ОписаниеОшибки());
	КонецПопытки;
...
Процедура СоздатьПриходныйСкладскойОрдер(ДляЗаполнения)
	...
	Если ТипЗнч(ДляЗаполнения.Ссылка) <>  Тип("ДокументСсылка.ПеремещениеТоваров") И НЕ ДляЗаполнения.Ссылка.Склад.ОрдерныйУчетПоПриходу Тогда
		Возврат;
	ИначеЕсли ТипЗнч(ДляЗаполнения.Ссылка) =  Тип("ДокументСсылка.ПеремещениеТоваров") И НЕ ДляЗаполнения.Ссылка.СкладПолучатель.ОрдерныйУчетПоПриходу Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ОбновитьПеремещениеТоваров(ДляЗаполнения)
	...
	Если ДляЗаполнения.Состояние = "ACCEPTED" Тогда
		Возврат;
	КонецЕсли; 
...
Процедура СоздатьДокументыПоИсходящимПеремещениям(ДляЗаполнения)
	...
	Если ПроверитьСтруктуруПодчиненности(КритерииОтбора.ДокументыПоПеремещениюТоваров, ДляЗаполнения.Ссылка, Тип("ДокументСсылка.РасходныйСкладскойОрдер"))
		И ВРЕГ(ДляЗаполнения.Состояние) = "SHIPPED" Тогда
		Возврат;	
	КонецЕсли;
	
	ОбновитьТекущееСостояниеПеремещения(ДляЗаполнения);
...
Функция СоздатьСериюНоменклатуры(Номенклатура, ДатаПроизводства, СрокГодности, РежимЗагрузкиДанных = Ложь,НаименованиеПоШаблону=истина)
	...
	Попытка
		...
		НовЭлемент.СтранаПроисхождения = НовЭлемент.Владелец.СтранаПроисхождения;			
		Если Номенклатура.ОсновнойПоставщик = ПепсикоХолдинг тогда
			Если НовЭлемент.НомерПартииПоставщика = "" Тогда
				НовЭлемент.НомерПартииПоставщика = "FGDUMMY";
			КонецЕсли; 
			НовЭлемент.Используется = Истина;
		КонецЕсли;
		Если НаименованиеПоШаблону Тогда
			НовЭлемент.Наименование = ОбщиеФункции._СтрШаблон_("%1 (%2)", Формат(ДатаПроизводства, "ДЛФ=D"), Формат(СрокГодности, "ДЛФ=D"));
		Иначе
			НовЭлемент.Наименование = НовЭлемент.СформироватьНаименование();
		Конецесли;
...
Функция СоздатьСписаниеТоваров(ТипОперации, Организация, Состав, ВладелецЗапасов)
	...
	Если ТипЗнч(ВладелецЗапасов) = Тип("СправочникСсылка.Контрагенты") Тогда
		ДокОбъект.Склад         = ЛогКомплекс_СОХ;
		ДокОбъект.Контрагент    = ВладелецЗапасов;
		ДокОбъект.ДоговорКонтрагента = _3PLСервер.НайтиПодходящийДоговорКонтрагента(ВладелецЗапасов, Организация, Перечисления.ВидыДоговоровКонтрагентов.ПриемНаХранение);
	Иначе
	...
	//{[DISTR-267]Боровик А.О, 2020-11-30 17:38:57
	//Попытка
	...
	//	Лог1С.Вывести(ТекстОшибки, СтатусСообщения.Внимание);
	//КонецПопытки;
	//} Боровик А.О, 2020-11-30 17:38:57
	
	Возврат ДокОбъект.Ссылка;
	
КонецФункции

Функция СоздатьОприходованиеТоваров(ТипОперации, Организация, Состав, ВладелецЗапасов)
	...
	Если ТипЗнч(ВладелецЗапасов) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		ДокОбъект.Контрагент    = ВладелецЗапасов;
		ДокОбъект.ДоговорКонтрагента = _3PLСервер.НайтиПодходящийДоговорКонтрагента(ВладелецЗапасов, Организация, Перечисления.ВидыДоговоровКонтрагентов.ПриемНаХранение);
		ДокОбъект.ОтветственноеХранение = Истина;
	Иначе
		ДокОбъект.Склад         = ЛогКомплекс;
	КонецЕсли; 
	ДокОбъект.Склад 			= ?(ТипЗнч(ВладелецЗапасов) = Тип("СправочникСсылка.Контрагенты"), ЛогКомплекс_СОХ, ЛогКомплекс);
	ДокОбъект.Организация		= Организация; 
	ДокОбъект.Подразделение 	= ДокОбъект.Автор.Подразделение;
	...
	//{[DISTR-267]Боровик А.О, 2020-11-30 17:39:18
	//Попытка
	//	// Документ с алкогольной продукцией проводится вручную
	//	Если НЕ ЕстьАлкоголь Тогда
	//		ДокОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Оперативный);
	...
	//	Лог1С.Вывести(ТекстОшибки, СтатусСообщения.Внимание);
	//КонецПопытки;
	//} Боровик А.О, 2020-11-30 17:39:18
	
	Возврат ДокОбъект.Ссылка;
	
КонецФункции

Функция СоздатьКорректировкуКачестваЗапасов(ТипОперации, Состав, ВладелецЗапасов)
	...
	//{[DISTR-267]Боровик А.О, 2020-11-30 17:39:39
	//Попытка
	//	// Документ с алкогольной продукцией проводится вручную
	//	Если НЕ ЕстьАлкоголь Тогда
	//		ДокОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Оперативный);
	...
	//	Лог1С.Вывести(ТекстОшибки, СтатусСообщения.Внимание);
	//КонецПопытки;
	//} Боровик А.О, 2020-11-30 17:39:39
...
Процедура СоздатьДокументыКорректировкиЗапасов(НачДата = Неопределено, КонДата = Неопределено)
	...
	Для каждого СтрТипОперации Из тзТипыОпераций Цикл
		...
		Если СтрТипОперации.ТипОперации = "+" Тогда
			...
			СоздатьОприходованиеТоваров(СтрТипОперации.ТипОперации, СтрТипОперации.Организация, Состав, СтруктураОтбора.ВладелецЗапасов);
			
		ИначеЕсли СтрТипОперации.ТипОперации = "-" Тогда 
			
			СоздатьСписаниеТоваров(СтрТипОперации.ТипОперации, СтрТипОперации.Организация, Состав, СтруктураОтбора.ВладелецЗапасов);
			
		ИначеЕсли СтрТипОперации.ТипОперации = "DATE" Тогда 
			
			ДокСписание = СоздатьСписаниеТоваров(СтрТипОперации.ТипОперации, СтрТипОперации.Организация, Состав, СтруктураОтбора.ВладелецЗапасов);
			
			ДокОприходование = СоздатьОприходованиеТоваров(СтрТипОперации.ТипОперации, СтрТипОперации.Организация, Состав, СтруктураОтбора.ВладелецЗапасов);
		...
		ИначеЕсли СтрТипОперации.ТипОперации = "STOCK" ИЛИ СтрТипОперации.ТипОперации = "SYSTEM" Тогда
			...
			СоздатьКорректировкуКачестваЗапасов(СтрТипОперации.ТипОперации, Состав, СтруктураОтбора.ВладелецЗапасов);
...
Процедура СоздатьТаблицуСопосталенныхПричинНедоборов()
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ПричиныВозвратовОтЭкспедиторов.Ссылка,
	|	ПричиныВозвратовОтЭкспедиторов.ВнешнийКодWMS
	|ИЗ
	|	Справочник.ПричиныВозвратовОтЭкспедиторов КАК ПричиныВозвратовОтЭкспедиторов
	|ГДЕ
	|	ПричиныВозвратовОтЭкспедиторов.ВнешнийКодWMS <> """"";
	
	ТаблицаСопосталенныхПричинНедоборов = Запрос.Выполнить().Выгрузить();
	
КонецПроцедуры
 
Функция ПолучитьПричинуНедобораПоВнешнемуКоду(ВнешнийКод)
	
	Причина = Справочники.ПричиныВозвратовОтЭкспедиторов.ПустаяСсылка();
	
	Если ТаблицаСопосталенныхПричинНедоборов = Неопределено Тогда
		СоздатьТаблицуСопосталенныхПричинНедоборов();
	КонецЕсли;
	
	СтрокаТабЗнач = ТаблицаСопосталенныхПричинНедоборов.Найти(Строка(ВнешнийКод), "ВнешнийКодWMS");
	
	Если СтрокаТабЗнач <> Неопределено Тогда
		Причина = СтрокаТабЗнач.Ссылка;	
	КонецЕсли; 
...
Процедура ПеренестиВозвратнуюТаруИзДокументаОтгрузки(ДокументОтбора, ДокументОтгрузки)
	
	ПорядокЛистОтбора = 1;
	ПорядокСерииОтбора = 1;
	
	Если ЗначениеЗаполнено(ДокументОтбора.ЛистОтбора) Тогда
		
		МаксИндекс = ДокументОтбора.ЛистОтбора.Количество() - 1;
		ПорядокЛистОтбора = ДокументОтбора.ЛистОтбора.Получить(МаксИндекс).Порядок;
		
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(ДокументОтбора.СерииОтбора) Тогда
		
		МаксИндекс = ДокументОтбора.СерииОтбора.Количество() - 1;
		ПорядокСерииОтбора = ДокументОтбора.СерииОтбора.Получить(МаксИндекс).Порядок;
		
	КонецЕсли;
	
	Для каждого СтрТара Из ДокументОтгрузки.ВозвратнаяТара Цикл
		
		НовСтр                         = ДокументОтбора.ЛистОтбора.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтр,  СтрТара, "Номенклатура, СерияНоменклатуры");

		НовСтр.ЗаказНаОтгрузку         = ДокументОтгрузки;
		НовСтр.Порядок                 = ПорядокЛистОтбора;
		НовСтр.КоличествоПлан          = СтрТара.Количество;
		НовСтр.КоличествоФакт          = СтрТара.Количество;
		НовСтр.Весовой                 = НовСтр.Номенклатура.Весовой;
		НовСтр.Качество                = Справочники.Качество.Стандарт;
		НовСтр.СерияНоменклатурыОтбора = НовСтр.СерияНоменклатуры;

		НовСтр2                        = ДокументОтбора.СерииОтбора.Добавить();
		ЗаполнитьЗначенияСвойств(НовСтр2, НовСтр);
		НовСтр2.ДатаПроизводства 	   = НовСтр2.СерияНоменклатурыОтбора.ДатаПроизводства;
		НовСтр2.Порядок                = ПорядокСерииОтбора;

		ПорядокЛистОтбора              = ПорядокЛистОтбора + 1;
		ПорядокСерииОтбора             = ПорядокСерииОтбора + 1;
		
	КонецЦикла; 
	
КонецПроцедуры

Функция ОпределитьСостояниеДокументаПоСтатусуОтветаWMS(СтатусОтвета)
	
	СостояниеДокументаОтгрузки = Перечисления.СостоянияДокументовОтгрузки.ПустаяСсылка();
	
	Если ВРЕГ(СтатусОтвета) = "SHIPPED" Тогда
		СостояниеДокументаОтгрузки = Перечисления.СостоянияДокументовОтгрузки.Отгружен;
	ИначеЕсли ВРЕГ(СтатусОтвета) = "READY_FOR_SHIPMENT" Тогда
		СостояниеДокументаОтгрузки = Перечисления.СостоянияДокументовОтгрузки.Отобран;
	ИначеЕсли ВРЕГ(СтатусОтвета) = "ACCEPTED" Тогда
		СостояниеДокументаОтгрузки = Перечисления.СостоянияДокументовОтгрузки.ВОтборе
	Иначе
		СостояниеДокументаОтгрузки = Перечисления.СостоянияДокументовОтгрузки.Создан;
	КонецЕсли;
	
	Возврат СостояниеДокументаОтгрузки;
	
КонецФункции

Процедура ОбновитьТекущееСостояниеПеремещения(ДляЗаполнения)
	
	ТекСостояние = ОпределитьСостояниеДокументаПоСтатусуОтветаWMS(ДляЗаполнения.Состояние);
	
	ЗаписьСуществует = Документы.ОтгрузкаТоваровУслуг.ПроверитьСостояниеДокумента(ДляЗаполнения.Ссылка, ТекСостояние);
	
	Если ЗаписьСуществует Тогда
		Возврат;
	КонецЕсли;
	
	НовРегистратор = Документы.РегистраторЗаписи.СоздатьНовыйДокумент("Регистрация состояния для документа " + ДляЗаполнения.Ссылка);
	
	НовРегистратор = НовРегистратор.ПолучитьОбъект();
	
	НаборЗаписей = НовРегистратор.Движения.СостояниеДокументовОтгрузки;
	
	НовЗапись                  = НаборЗаписей.Добавить();
	НовЗапись.Состояние        = ТекСостояние;
	НовЗапись.Период		   = НовРегистратор.МоментВремени().Дата;
	НовЗапись.ДокументОтгрузки = ДляЗаполнения.Ссылка;
...
Функция ПреобразоватьДанныеВМС_В_1С(Состав)
	...
	втТабЗнач.Колонки.Добавить("ВладелецЗапасов",           Новый ОписаниеТипов("СправочникСсылка.Кластеры, СправочникСсылка.Контрагенты"));
	...
	Для каждого ЭлКоллекции Из Состав Цикл
		...
		СтруктураЗаписи.Вставить("СерияНоменклатуры", 	ПолучитьСериюНоменклатурыПоДатеПроизводства(СтруктураЗаписи.Номенклатура, СтруктураЗаписи.ДатаПроизводства, ДопПараметрыДляЕГАИС,,истина,СтруктураЗаписи.СрокГодности));
		...
		СтруктураЗаписи.Вставить("ВладелецЗапасов", ?(СтруктураЗаписи.Номенклатура.ОтветХранение,  СтруктураЗаписи.Номенклатура.ОсновнойПоставщик, СтруктураЗаписи.Номенклатура.ДистрибьюторскийКонтракт.Кластер));	
		
		Если Состав.Колонки.Найти("OwnerCode") <> Неопределено Тогда
			
			ПереданныйВладелецЗапасов = Справочники.Кластеры.ПолучитьСсылку(Новый УникальныйИдентификатор(ЭлКоллекции.OwnerCode));
			
			Если СокрЛП(ПереданныйВладелецЗапасов.Код) = "" Тогда
				ПереданныйВладелецЗапасов = Справочники.Контрагенты.ПолучитьСсылку(Новый УникальныйИдентификатор(ЭлКоллекции.OwnerCode));	
			КонецЕсли; 
			
			Если ЗначениеЗаполнено(ПереданныйВладелецЗапасов) Тогда
				СтруктураЗаписи.Вставить("ВладелецЗапасов", ПереданныйВладелецЗапасов);	
			КонецЕсли; 
...
Процедура ПолучитьОтветыWMSПоЗаявкам()
	...
	Если НЕ Автотесты Тогда
		...
		Попытка
			Результат = ВыполнитьКоманду(ОбщиеФункции._СтрШаблон_("exec GetDocnum @filter = '%1'", ОбщиеФункции._СтрШаблон_("docnum.DOC_SENDER='%1' and docnum.DOC_RECEIVER='%2' and docnum.CHECKED_BY_PI is null and docnum.RecordDate >= ''%3''", "'WMS'", "'1С'", Формат(ТекущаяДата() - 36000, "ДФ='dd-MM-yyyy HH:mm:ss'"))));
		Исключение
...
Функция ПолучитьСериюНоменклатурыПоДатеПроизводства(Владелец, ДатаПроизводства, ДопПараметрыДляЕГАИС = Неопределено, ДопПараметрыМеркурий = Неопределено,Создавать=ложь,СрокГодности=неопределено, Качество = Неопределено)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	влСерии.СерияНоменклатуры КАК СерияНоменклатуры
	|ИЗ
	|	(ВЫБРАТЬ
	|		влЗапрос.СерияНоменклатуры КАК СерияНоменклатуры,
	|		ВЫБОР
	|			КОГДА ЗапасыНаСкладах.СерияНоменклатуры ЕСТЬ NULL
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ КАК ЕстьСерияНаОстатке
	|	ИЗ
	|		(ВЫБРАТЬ
	...
	|						И ВЫБОР
	|							КОГДА &Качество <> НЕОПРЕДЕЛЕНО
	|								ТОГДА Качество = &Качество
	|							ИНАЧЕ ИСТИНА
	|						КОНЕЦ) КАК ЗапасыНаСкладах
	|			ПО влЗапрос.СерияНоменклатуры = ЗапасыНаСкладах.СерияНоменклатуры) КАК влСерии
	|
	|УПОРЯДОЧИТЬ ПО
	|	влСерии.ЕстьСерияНаОстатке УБЫВ";
	...
	Иначе
		Если Создавать Тогда
			Если ЗначениеЗаполнено(ДатаПроизводства) Тогда
				СерияНоменклатуры = СоздатьСериюНоменклатуры(Владелец, ДатаПроизводства, ?(СрокГодности=неопределено,Дата(1,1,1),СрокГодности));
			Иначе	
				Лог1С.Вывести(ОбщиеФункции._СтрШаблон_("Для номенклатуры '%1' с моделью складского учета '%2' не удалось создать серию по причине: не указана дата производства '%3'", 
...
Процедура Инициализация()
	...
	КоличествоДнейДляКонтроляОтменыПроведения = Справочники.ВнешниеСистемы.ПолучитьЗначенияКлюча(ВнешСистема, "НеОтменятьПроведениеЕслиДатаДокументаМеньше");
	
	Если НЕ ЗначениеЗаполнено(КоличествоДнейДляКонтроляОтменыПроведения) Тогда
		КоличествоДнейДляКонтроляОтменыПроведения = 30;
	КонецЕсли; 
	
	ПепсикоХолдинг = Справочники.ВнешниеСистемы.ПолучитьЗначенияКлюча(ВнешСистема, "ПепсикоХолдинг");
