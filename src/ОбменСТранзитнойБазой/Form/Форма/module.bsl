...
Процедура КоманднаяПанель1ПосмотретьЖурналЗаявок(Кнопка)
	...
	НовЭлементОтбора.ПравоеЗначение = ОбщиеФункции.ДобавитьДень(НачалоДня(ТекущаяДата()), -2) ;
	НовЭлементОтбора.Использование = Истина;	
	
	ЭлементПорядка = ФормаЖурналаСостоянийЗаявок.Список.Порядок.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
	ЭлементПорядка.Поле = Новый ПолеКомпоновкиДанных("ДатаЗаписи");
...
Процедура КоманднаяПанель1ВыгрузитьПервичку(Кнопка)
	Форма = ПолучитьФорму("фПервичнаяВыгрузкаСправочников");
