...
Процедура ОсновныеДействияФормыВыполнить(Кнопка)
	...
	НачатьТранзакцию();
	Попытка
		ВладелецФормы.ОбработкаОбъект.ПолучитьЗапасыНаСкладеИзWMS(ПараметрыОтчета, ТолькоПросмотр, АдресСпискаДокументов, АдресТаблицыДляОбработкиОшибок);
	Исключение
		ОтменитьТранзакцию();
...
Процедура ЗаполнитьДополнительныеПараметры(ДополнительныеПараметры)
	...
	ДополнительныеПараметры = Новый Структура("СписокДокументов, ОбработкаОшибокПоЗагрузкеОписей, НовыеСерии", 
		Новый СписокЗначений, тзОбработкиОшибок, тзНовыеСерии);
...
Процедура ЗаполнитьВидыОстатков(Отказ)
	...
	Пока НЕ Результат.EOF Цикл
		...
		ВидыОстатков.Добавить(Результат.Fields("tid").Value, Результат.Fields("NameRU").Value);
...
Процедура ЗаполнитьВладельцевЗапасов(Отказ)
	...
	Пока НЕ Результат.EOF Цикл
		...
		ВладельцыЗапасов.Добавить(Результат.Fields("tid").Value, Результат.Fields("FullName").Value);
