...
Процедура ОбновитьПоставки() Экспорт
	...
	Попытка
		...
		Для каждого СтрТаб Из ТаблицаЗаявок Цикл
			...
			ТелоЗапроса = СформироватьТелоЗапросаДляОбновленияПоставки(СтрТаб.Объект, СокрЛП(СтрТаб.НомерПоставки), СтрТаб.Идентификатор);
...
Процедура СоздатьДокументы(ТаблицаДанных)
	...
	Для каждого СтрТаб Из ТаблицаДанных Цикл
		...
		Если СтрТаб.ТребуемоеДействие = "Добавление" Тогда
			...
			Если СтрТаб.ТипДокумента = "J" И СтрТаб.Контрагент.ИНН = "5504086398" Тогда // ООО "ТД "Шкуренко"
				Попытка
					ОбновитьВыкупСХранения(ПреобразоватьСтрокуТабЗначВСтруктуру(СтрТаб, ТаблицаДанных.Колонки));
				Исключение
					Сообщить(ОбщиеФункции._СтрШаблон_("Не удалось создать документы отгрузки по заказу №'%1' по причине: 
					|%2", СтрТаб.НомерВходящегоДокумента, ОписаниеОшибки()));
				КонецПопытки;
...
Функция ПолучитьЗаявкиНаОбновлениеПоставок()
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Объект,
	|	ВЫРАЗИТЬ(ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Текст КАК СТРОКА(25)) КАК НомерПоставки,
	...
	|
	|ВЫБРАТЬ
	|	ЗаявкиНаОбновлениеСтатусов005.Объект,
	|	СвойстваОбъекта.Значение,
	|	""shipmentupdate_005""
	|ИЗ
	|	Документ.ИнвентаризацияДоставки КАК ИнвентаризацияДоставки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций КАК ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций
	|		ПО ИнвентаризацияДоставки.Ссылка.ДокументОтгрузки = ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Объект
	|			И (ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Источник = &Приемник)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияСвойствОбъектов КАК СвойстваОбъекта
	|		ПО ИнвентаризацияДоставки.Ссылка.ДокументОтгрузки = СвойстваОбъекта.Объект
	|			И (СвойстваОбъекта.Свойство = &НомерПоДаннымПоставщика)
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			ЖурналЗаявок.Объект КАК Объект,
	|			ВЫРАЗИТЬ(ЖурналЗаявок.Текст КАК СТРОКА(36)) КАК НомерТранспортировки
	|		ИЗ
	|			РегистрСведений.ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций КАК ЖурналЗаявок
	|		ГДЕ
	|			ЖурналЗаявок.Приемник = &Приемник
	|			И ЖурналЗаявок.Идентификатор = ""shipmentupdate_005""
	|			И ЖурналЗаявок.Статус = ЗНАЧЕНИЕ(Справочник.СтатусыЗаявок.Новая)) КАК ЗаявкиНаОбновлениеСтатусов005
	|		ПО (СвойстваОбъекта.Значение = ЗаявкиНаОбновлениеСтатусов005.НомерТранспортировки)
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаявкиНаОбновлениеСтатусов005.Объект,
	|	СвойстваОбъекта.Значение";
...
Функция СформироватьТелоЗапросаДляОбновленияПоставки(Объект, НомерПоставки, ИдентификаторОперации)
	...
	Запись.ЗаписатьНачалоОбъекта();
		...
		Иначе
			...
			ЗаписатьСвойствоИЗначениеJSON(Запись, "startTimeZone", 		ЧасовойПояс);
			ЗаписатьСвойствоИЗначениеJSON(Запись, "endDate", 			Формат(ТекущаяДатаСеанса(), "ДФ=yyyyMMdd"));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "endTime", 			Формат(НачалоДня(ТекущаяДатаСеанса()) + 72000, "ДФ=HHmmss"));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "endTimeZone", 		ЧасовойПояс);
			Запись.ЗаписатьКонецОбъекта();
...
Процедура СоздатьИнвентаризациюДоставкиСводную(ДляЗаполнения)
	...
	Для каждого СтрТаб Из ДляЗаполнения.Товары Цикл
		...
		НовСтр = ДокументОбъект.Состав.Добавить();
...
Функция ПолучитьСведенияОВычерках(МассивДокументов)
	...
	|	Вычерки.Коэффициент,
	|	Вычерки.Передано КАК Количество
	|ИЗ
