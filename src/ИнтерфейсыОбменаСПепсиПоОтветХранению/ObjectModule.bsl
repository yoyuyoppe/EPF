...
Процедура ПроверкаЗаполнения()
	...
	|ОсновнойКодПричиныНеполнойКомплектации;ОсновнойСклад;КачествоПретензия", ";");
...
Процедура ТестированиеИнтерфейсов()
	...
	Структура = Новый Структура("Склад, СкладПолучатель, ТипДвижения, Номенклатура, СерияНоменклатуры, ЕдиницаИзмерения, Качество, КачествоНовое, Количество");
	
	Структура.Вставить("Склад", СвойстваПартнера.Склад);
	Структура.Вставить("СкладПолучатель", Справочники.Качество.НайтиПоНаименованию("Претензия", Истина));
	Структура.Вставить("Номенклатура", Справочники.Номенклатура.НайтиПоКоду("Ц060518514"));
	Структура.Вставить("СерияНоменклатуры", Справочники.СерииНоменклатуры.НайтиПоНаименованию("13.05.2012 (31.12.2029), FGDUMMY", Истина,, Структура.Номенклатура));
	Структура.Вставить("ЕдиницаИзмерения", Структура.Номенклатура.ЕдиницаХраненияОстатков);
	Структура.Вставить("Качество", Справочники.Качество.Стандарт);
	Структура.Вставить("КачествоНовое", Справочники.Качество.Стандарт);
	Структура.Вставить("Количество", 100);
	Структура.Вставить("ТипДвижения", ?(Структура.Количество < 0, "ZF3", "ZF4"));
...
Функция СформироватьТелоЗапросаПоРасхождениямПриПересчете(ВыборкаПоТипуОперации)
	...
	СлужебныйРегистратор = Документы.РегистраторЗаписи.СоздатьНовыйДокумент("Расхождения при пересчете");
	
	_3PLСервер.ДобавитьЗаписьПоДокументуВЖурналОпераций(СлужебныйРегистратор, "1С", Строка(Партнер), Справочники.СтатусыЗаявок.Новая, "РасхожденияПриПересчете",,, ТекстОперации);
...
Функция ПолучитьДанныеПоДвижениямКорректировкиОстатков()
	...
	|	&КачествоПретензия КАК СкладПолучателя,
