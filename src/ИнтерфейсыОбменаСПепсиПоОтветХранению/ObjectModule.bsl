...
Процедура СоздатьОперациюНаОтправкуРасхожденийПриПересчете(МассивРасхождений, ТипОперации)
	...
	Запись.ЗаписатьНачалоОбъекта();
	ЗаписатьСвойствоИЗначениеJSON(Запись, "trcode", ТипОперации);
	Запись.ЗаписатьКонецОбъекта();
	...
	Для каждого Элемент Из МассивРасхождений Цикл
		...
		Запись.ЗаписатьНачалоОбъекта();
			ЗаписатьСвойствоИЗначениеJSON(Запись, "itemnum", ДобавитьЛидирующиеНулиВКодТовара(Элемент.Номенклатура.Артикул));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "lot", Элемент.СерияНоменклатуры.НомерПартииПоставщика);
			ЗаписатьСвойствоИЗначениеJSON(Запись, "qty", ?(Элемент.Количество < 0, -Элемент.Количество, Элемент.Количество));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "baseuom", ОпределитьЕдиницуИзмеренияПепси(Элемент.ЕдиницаИзмерения));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "movtype", Элемент.ТипДвижения);
			ЗаписатьСвойствоИЗначениеJSON(Запись, "storloc", _3PLСервер.ОпределитьСкладПоклажедателя(Партнер, Элемент.Склад));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "newstorloc", _3PLСервер.ОпределитьСкладПоклажедателя(Партнер, Элемент.СкладПолучатель));
	...
	Если ТипПоставки <> "J" Тогда
		...
		Для каждого СтрТабЧасть Из ДокументСсылка.Товары Цикл
			...
			Если СтрТабЧасть.Количество <> КоличествоФакт Тогда
				...
				СтруктураРасхождения.СкладПолучатель = ДокументСсылка.Склад;
