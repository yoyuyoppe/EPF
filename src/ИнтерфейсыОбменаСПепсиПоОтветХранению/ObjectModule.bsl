...
Процедура Тест_ПолучитьДокументы()
	...
	Текст = Новый ТекстовыйДокумент;
	Текст.Прочитать("D:\1С (Разное)\МОЗАЙКА\CheckIn.json");
...
Функция ЕстьЗаявкаНаОперациюОбновленияПоставок(НомерТранспортировки)
	
	Возврат ТаблицаНовыхЗаявокНаОбновлениеСтатусовПоставки.Найти(НомерТранспортировки, "НомерТранспортировки") <> Неопределено;
...
Функция ПолучитьЗаявкиНаОбновлениеПоставок()
	...
	|		ПО (СвойстваОбъектов.Объект = ИнвДоставки.ДокументОтгрузки)
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(СвойстваОбъектов.Объект) = ТИП(Документ.ОтгрузкаТоваровУслуг)
	|	И ИнвДоставки.Ссылка ЕСТЬ NULL
	...
	Если ЗначениеЗаполнено(ЗаблокированныеТранспортировки) Тогда
		...
		ОжидаемыйСтатусТранспортировкиДо005 = "Выполнен или Не выполнен";
...
Процедура СоздатьЗаявкуНаОперациюОбновленияПоставок(Объект, МассивТранспортировок)
	...
	Если МассивТранспортировок.Найти(НомерТранспортировки) = Неопределено Тогда
		...
		Если ЕстьЗаявкаНаОперациюОбновленияПоставок(НомерТранспортировки) Тогда
			Возврат;
		КонецЕсли;  
...
Процедура СоздатьИнвентаризациюДоставкиСводную(ДляЗаполнения)
	...
	ОбщиеФункции.УстановитьЗначениеСвойства(Новый Структура("Объект, Свойство, Значение", ДокументОбъект.Ссылка, СвойстваПартнера.Свойство_НомерВходящегоДокумента, ДляЗаполнения.НомерТранспортировки));
