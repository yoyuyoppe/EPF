	...
	Для каждого СтрТаб Из ТаблицаДокументов Цикл
		...
		Попытка
			...
			Если Ответ.КодСостояния > 200 И Ответ.КодСостояния < 300 Тогда
				...
				Если ТипЗнч(СтрТаб.Объект) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
...
Процедура ПроверкаЗаполнения()
	...
	|ТехническоеИмяМестаОтгрузки;КаталогХраненияПечатныхФорм;НомерВиртуальнойПартии;ГруппаХранения;ВремяНачалаМаршрута;ВремяОкончанияМаршрута;СреднееВремяПосещения;Углекислота;Баллон;ОсновнойДоговорПокупка", ";");
...
Функция ПолучитьДанныеДляПереводаЦифровыхПартийНаВиртуальную(ЗаказыПоставщику)
	...
	|	ЗаказПоставщику.Ссылка.Склад КАК Склад,
	|	ЗаказПоставщику.Ссылка.Склад КАК СкладПолучатель,
	...
	|	И ЗаказПоставщику.СерияНоменклатуры.НомерПартииПоставщика <> &НомерВиртуальнойПартии
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата
	|ИТОГИ
...
Процедура ПеревестиЦифровыеПартииНаВиртуальнуюВСап(ЗаказПоставщику)
	...
	ВыборкаДанных = ПолучитьДанныеДляПереводаЦифровыхПартийНаВиртуальную(ЗаказПоставщику);
	...
	Пока ВыборкаДанных.Следующий() Цикл
		...
		Попытка
			...
			Если Ответ.КодСостояния > 200 И Ответ.КодСостояния < 300 Тогда
				...
				РегистраторЗаписи = Документы.РегистраторЗаписи.СоздатьНовыйДокумент(ВыборкаДанных.Ссылка);
				
				_3PLСервер.ДобавитьЗаписьПоДокументуВЖурналОпераций(РегистраторЗаписи.Ссылка, "1С", Строка(Партнер), Справочники.СтатусыЗаявок.УспешноОбработана,
	...
	Пока ВыборкаДетальныхЗаписей.Следующий() цикл
		...
		Запись.ЗаписатьНачалоОбъекта();
			...
			Если  ВыборкаПоТипуОперации.ТипДвижения = "311"
				И НЕ ВыборкаДетальныхЗаписей.Номенклатура.ПринадлежитЭлементу(Молочка)
				И ВыборкаДетальныхЗаписей.ЭтоПересорт
				И ВыборкаДетальныхЗаписей.СерияНоменклатуры.НомерПартииПоставщика <> "FGDUMMY" Тогда
				
				ЗаписатьСвойствоИЗначениеJSON(Запись, "newbatch", "FGDUMMY");
				
				Если МассивПартийДляПереводаНаВиртуальную.Найти(ВыборкаДетальныхЗаписей.СерияНоменклатуры) = Неопределено Тогда
					МассивПартийДляПереводаНаВиртуальную.Добавить(ВыборкаДетальныхЗаписей.СерияНоменклатуры);
				КонецЕсли; 
...
Функция ПолучитьСерииОтбора(ДокументСсылка)
	...
	|	ОтборЗапасовНаСкладеСерииОтбора.КоличествоПлан,
...
Функция СформироватьТелоЗапросаДляОбновленияПоставки(Объект, НомерТранспортировки, ИдентификаторОперации)
	...
	Запись.ЗаписатьНачалоОбъекта();
		...
		Если ИдентификаторОперации = "shipmentupdate_004" Тогда
			...
			ЗаписатьСвойствоИЗначениеJSON(Запись, "endDate", 			Формат(ВремяСборкиТранспортировки, "ДФ=yyyyMMdd"));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "endTime", 			Формат(ВремяСборкиТранспортировки + (СмещениеВСекундах + СмещениеВМинутах), "ДФ=HHmmss"));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "endTimeZone", 		ЧасовойПояс);
			Запись.ЗаписатьКонецОбъекта();
		...
		ИначеЕсли ИдентификаторОперации = "shipmentupdate_005_start" Тогда 
			...
			ЗаписатьСвойствоИЗначениеJSON(Запись, "startDate", 			Формат(МаршрутСсылка.НачалоМаршрута, "ДФ=yyyyMMdd"));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "startTime", 			Формат(МаршрутСсылка.НачалоМаршрута, "ДФ=HHmmss"));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "startTimeZone", 		ЧасовойПояс);
			Запись.ЗаписатьКонецОбъекта();
	...
	Для каждого Элемент Из Данные Цикл
		...
		Для каждого СтрДок Из СоставДокумента Цикл
			...
			Если ЕстьГазВПоставке И СвойстваПартнера.Свойство("Баллон") Тогда
				...
				СтруктураСтрокиДокумента.Вставить("Качество", Справочники.Качество.Стандарт);
