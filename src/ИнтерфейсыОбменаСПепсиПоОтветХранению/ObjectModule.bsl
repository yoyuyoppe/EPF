...
Функция СформироватьТелоЗапросаДляОтправкиМаршрута(ДокументСсылка)
	 ...
	 Если НЕ ЗначениеЗаполнено(ВремяНачалаМаршрута) Тогда
		 ВремяНачалаМаршрута = ДокументСсылка.ДатаЭкспедиции + (СвойстваПартнера.ВремяНачалоМаршрута - Дата(1,1,1));
	 КонецЕсли; 
	
	 ВремяОкончанияМаршрута = 	ДокументСсылка.ДатаЭкспедиции + (ВыборкаПоМаршруту.ВремяОкончанияМаршрута - Дата(1,1,1));
	 Если НЕ ЗначениеЗаполнено(ВремяОкончанияМаршрута) Тогда
