...
Перем ДатаПоследнегоОбмена; // переделать потом. Для каждого интерфейса будет отдельная дата
...
Процедура ОтправитьРасхожденияПриПересчете()
	...
	ОписаниеМетода = МетодыОбменаСПартнером.МетодОбмена_РасхожденияПриПересчете;
	...
	Попытка
		...
		ТелоЗапроса = СформироватьТелоЗапросаПоРасхождениямПриПересчете();
		...
		Если Ответ.КодСостояния > 200 И Ответ.КодСостояния < 300 Тогда
			_3PLСервер.ОперацияВыполненаУспешно(СтруктураМетода.Действие, Строка(Партнер), ТелоЗапроса);
...
Процедура ПодтвердитьПоставки() Экспорт
	...
	Для каждого СтрТаб Из ТаблицаДокументов Цикл
		...
		Попытка
			...
			Если Ответ.КодСостояния > 200 И Ответ.КодСостояния < 300 Тогда
				_3PLСервер.ПоставкаПодтвержденаУспешно(СтрТаб.Объект, Строка(Партнер), ТелоЗапроса);
				СоздатьОперациюНаОтправкуРасхожденийПриПересчете(МассивРасхождений, "03");
			ИначеЕсли Ответ.КодСостояния = 401
				И ТипЗнч(Данные) = Тип("Структура")
				И Данные.Свойство("detail")
				И Данные.detail = "Full authentication is required to access this resource" Тогда 
...
Процедура ПроверкаЗаполнения()
	...
	|ОсновнойКодПричиныНеполнойКомплектации;КодСклада", ";");
...
Процедура ЗаписатьСвойствоИЗначениеJSON(Запись, ИмяСвойства, пЗначение = Неопределено) Экспорт
	...
	Если пЗначение <> Неопределено Тогда
		Запись.ЗаписатьЗначение( ?(ТипЗнч(пЗначение) = Тип("Дата"), ЗаписатьДатуJSON(пЗначение, ФорматДатыJSON.ISO), Строка(пЗначение)) );
	КонецЕсли;
...
Процедура СоздатьЗапросДляМетода(СтруктураМетода, Запрос)
	// Укажаем обязательные параметры
	ПараметрыЗапроса = ОбщиеФункции._СтрШаблон_("?requiredTimeStamp.greaterThan=%1", Формат(СвойстваПартнера.ДатаПоследнегоОбмена, "ДФ='yyyy-MM-dd HH:mm:ss'"));
...
Функция УдалитьЛидирущиеНулиИзКодаТовара(Код)
...
Процедура ТестированиеИнтерфейсов()
	...
	//Текст.Прочитать("D:\1С (Разное)\МОЗАЙКА\shipment.json");
	//
	...
	ПроверкаЗаполнения();
	//
	//ЗаполнитьНомераТранспортировок(ОпределитьДокументыДляТранспортировки(ПреобразоватьДанныеПоТранспортировкамВТаблицуЗначений(РезультатЗапроса)));
		
	Сообщить(СформироватьТелоЗапросаПоТекущимОстаткам());
...
Функция СформироватьТелоЗапросаПоТекущимОстаткам()
	...
	Версия = "000";
	ЛогическийСклад = СвойстваПартнера.КодСклада;
	Завод = СвойстваПартнера.КодДистрибьютора;
	ЛогическийСкладБлокировки = СвойстваПартнера.КодСкладаБлокировки;
	...
	Для каждого СтрТаб Из ТаблицаОстатков Цикл
		...
		ЗаписатьСвойствоИЗначениеJSON(Запись, "skuid",         СтрТаб.Номенклатура.Артикул);
		
		ЗаписатьСвойствоИЗначениеJSON(Запись, "plant",         Завод);
		
		ЗаписатьСвойствоИЗначениеJSON(Запись, "storloc",       ?(СтрТаб.Качество <> КачествоДляБлокировки, ЛогическийСклад, ЛогическийСкладБлокировки));
		...
		ЗаписатьСвойствоИЗначениеJSON(Запись, "availableqty", ?(СтрТаб.Качество = Справочники.Качество.Стандарт, Формат(СтрТаб.КоличествоОстаток, "ЧРД=.; ЧГ=0"), "0"));
		
		ЗаписатьСвойствоИЗначениеJSON(Запись, "qiqty", ?(СтрТаб.Качество <> Справочники.Качество.Стандарт 
			И СтрТаб.Качество <> КачествоДляБлокировки, Формат(СтрТаб.КоличествоОстаток, "ЧРД=.; ЧГ=0"), "0"));
		
		ЗаписатьСвойствоИЗначениеJSON(Запись, "blockedqty", ?(СтрТаб.Качество = КачествоДляБлокировки, Формат(СтрТаб.КоличествоОстаток, "ЧРД=.; ЧГ=0"), "0"));
...
Функция СформироватьТелоЗапросаПоРасхождениямПриПересчете()
...
Процедура СоздатьОперациюНаОтправкуРасхожденийПриПересчете(МассивРасхождений, ТипДвижения)
	...
	Запись.ЗаписатьНачалоОбъекта();
	ЗаписатьСвойствоИЗначениеJSON(Запись, "trcode", ТипДвижения);
	Запись.ЗаписатьКонецОбъекта();
	
	ЗаписатьСвойствоИЗначениеJSON(Запись, "invmovdetail");
	Запись.ЗаписатьНачалоОбъекта();
	Для каждого Элемент Из МассивРасхождений Цикл
...
Функция СформироватьТелоЗапросаДляПодтвержденияПоставки(ДокументСсылка, ДокументПодтверждения, ТипОперации, МассивРасхождений)
	...
	Если ТипПоставки <> "J" Тогда
		...
		Для каждого СтрТабЧасть Из ДокументСсылка.Товары Цикл
			...
			ЗаписатьСвойствоИЗначениеJSON(Запись, "linenum",    ОбщиеФункции._СтрШаблон_("%10", СтрТабЧасть.НомерСтроки));
			ЗаписатьСвойствоИЗначениеJSON(Запись, "itemnum",    СтрТабЧасть.Номенклатура.Артикул);
				...
				Запись.ЗаписатьНачалоОбъекта();
					...
					ЗаписатьСвойствоИЗначениеJSON(Запись, "lot",        СтрТабЧасть.СерияНоменклатуры.НомерПартииПоставщика); 
				Запись.ЗаписатьКонецОбъекта();
			...
			Если СтрТабЧасть.Количество <> КоличествоФакт Тогда
				СтруктураРасхождения = Новый Структура("Номенклатура, СерияНоменклатуры, Разница");
				ЗаполнитьЗначенияСвойств(СтруктураРасхождения, СтрТабЧасть);
				СтруктураРасхождения.Разница = СтрТабЧасть.Количество - КоличествоФакт;
		...
		Для каждого СтрДок Из СоставДокумента Цикл
			...
			// жесткое резервирование по партии
				...
				Если СерияНоменклатуры.Пустая() Тогда
					...
					Попытка
						_3PLСервер.СоздатьСериюНоменклатуры(Новый Структура("Владелец, ДатаПроизводства, СрокГодности, НомерПартииПоставщика, Комментарий",
							СтрТоварПоклажедателя.Ссылка, ПрочитатьДату(СтрДок.prodDate, "dd.MM.yyyy"), ПрочитатьДату(СтрДок.sled, "dd.MM.yyyy"), СтрДок.lot, ОбщиеФункции._СтрШаблон_("#%1. Создана обменом по договору 3PL", СвойстваПартнера.КодДистрибьютора)), СерияНоменклатуры);
					Исключение
	...
	Для каждого Элемент Из Данные Цикл
		...
		Для каждого Единица Из СведенияОТоваре.itemConfigUOMS Цикл
			...
			Если (СведенияОТоваре.itemTypTxt = "ZFIN" И ЗначениеЗаполнено(Единица.barCode))
				ИЛИ (СведенияОТоваре.itemTypTxt = "ZEMT" И НЕ ЗначениеЗаполнено(Единица.barCode) И Единица.uomTypTxt = "EA") Тогда
...
Партнер = Справочники.КонтурEDI_ДополнительныеСправочники.НайтиПоКоду("000000097");

//ТестированиеИнтерфейсов();
