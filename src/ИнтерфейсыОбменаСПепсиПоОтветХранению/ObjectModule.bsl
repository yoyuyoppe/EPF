...
Процедура ПеревестиЦифровыеПартииНаВиртуальнуюВСап(ЗаказПоставщику) Экспорт
...
///Антропов Е.П. от 14.10.2024 УЗ №000000452
Функция ЕстьПомарочныйУчет(ДокументСсылка)
	ЕстьПУ = Ложь;
	...
	Запрос.Текст =  	
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(КодыМаркировки.КодМаркировки) > 0 КАК ЕстьПУ
	|ИЗ
	...
	|		ПО ОТУ.Ссылка = КодыМаркировки.ЗаказНаОтгрузку
	|ГДЕ
	|	ОТУ.Ссылка В (&ДокументСсылка)
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(КодыМаркировки.КодМаркировки) > 0"; 
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура СоздатьИнвентаризациюДоставкиСводную(ДляЗаполнения)
	...
	Если СвойстваПартнера.Свойство("ОжидатьПодтвержденияUtraceПоПУ") И СвойстваПартнера.ОжидатьПодтвержденияUtraceПоПУ Тогда 
		ЕстьПУ = ЕстьПомарочныйУчет(ДокументОбъект.ДокументыОтгрузки.ВыгрузитьКолонку("ДокументОтгрузки"));
		Если ЕстьПУ Тогда 
...
Функция ПреобразоватьДанныеПоДокументамВТаблицуЗначений(Данные)
	...
	Для каждого Элемент Из Данные Цикл
		...
		Для каждого СтрДок Из СоставДокумента Цикл
			...
			СтрокаСоСкладомПоклажедателя = ТаблицаСкладовПоклажедателей.Найти(СтрДок.storLoc, "Код");
...
Процедура Тест_ПеревестиПартииНаВиртуальную()
	...
	ЗаказПоставщика = Документы.ЗаказПоставщику.НайтиПоНомеру("ТШО0003426", ТекущаяДатаСеанса());
