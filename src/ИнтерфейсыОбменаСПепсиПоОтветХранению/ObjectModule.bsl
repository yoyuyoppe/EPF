...
Процедура ПроверкаЗаполнения()
	...
	|ТехническоеИмяМестаОтгрузки;КаталогХраненияПечатныхФорм;НомерВиртуальнойПартии;ГруппаХранения;ВремяНачалаМаршрута;ВремяОкончанияМаршрута;СреднееВремяПосещения;Углекислота;Баллон", ";");
...
Функция ПолучитьДанныеПоДвижениямКорректировкиОстатков()
	...
	|		""311"" КАК ТипДвижения,
	|		МАКСИМУМ(ВЫБОР
	|				КОГДА Состав2.Номенклатура ЕСТЬ NULL
	|					ТОГДА ЛОЖЬ
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ) КАК ЭтоПересорт
	|	ИЗ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйСкладскойОрдер.Товары КАК Состав2
	|			ПО (Док.Ссылка = Состав2.Ссылка)
	|				И ЗаказПоставщику.Номенклатура = Состав2.Номенклатура
