...
Процедура ОбновитьПартии(ТаблицаДанных)
	...
	Если ЗначениеЗаполнено(ТаблицаДанных) Тогда
		Сообщить("Выполняется обновление / добавление партий", СтатусСообщения.Информация);	
	КонецЕсли; 
	...
	Для каждого СтрТаб Из ТаблицаДанных Цикл
		...
		Если СтрТаб.ТребуемоеДействие = "Добавление" Тогда
			...
			КонецПопытки;
		КонецЕсли;
		// Временно отключим обновление, чтобы не перезаписать серии, созданные ранее	
		//Если СтрТаб.ТребуемоеДействие = "Обновление" Тогда
		//	Попытка
...
Функция ПреобразоватьДанныеПоПартиямВТаблицуЗначений(Данные)
	...
	Если ЗначениеЗаполнено(ТабЗнач) Тогда
		Сообщить("Данные по партиям получены", СтатусСообщения.Информация);
	Иначе 
		Сообщить("Новые данные по партиям не найдены!!!", СтатусСообщения.Информация);
	КонецЕсли; 
