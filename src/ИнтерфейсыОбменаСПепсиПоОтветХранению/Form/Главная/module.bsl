...
Процедура ЗаказыПоставщикамПриАктивизацииСтроки(Элемент)
	ТекущаяСтрока = ЭлементыФормы.ЗаказыПоставщику.ТекущиеДанные;
...
Процедура ОбновитьТаблицуРабочегоМестаОператора()
	...
	Если ИмяТекТаблицы = "ЗаявкиПрочее" Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИзменитьСтатус(ТипЗаявки)
	
	ФормаВыбора = Справочники.СтатусыЗаявок.ПолучитьФормуВыбора();
	ВыбЗначение = ФормаВыбора.ОткрытьМодально(20);
	
	Если ЗначениеЗаполнено(ВыбЗначение) Тогда
		
		ИмяТекТаблицы = ЭлементыФормы.ПанельОбъектовОбмена.ТекущаяСтраница.Имя;
		
		СтрокаТабЧасти = ЭлементыФормы[ИмяТекТаблицы].ТекущаяСтрока;
		_3PLСервер.ДобавитьЗаписьПоДокументуВЖурналОпераций(СтрокаТабЧасти.ДокументСсылка, Строка(Партнер), "1С", ВыбЗначение, ТипЗаявки, ТекущаяДатаСеанса());
		
		СтрокаТабЧасти.Статус = ВыбЗначение;
		
	КонецЕсли; 
	
КонецПроцедуры

Процедура КоманднаяПанельОТУкнИзменитьСтатусОТУ(Кнопка)
	
	ИзменитьСтатус("Исходящая");
	
КонецПроцедуры

Процедура КоманднаяПанельЗаказыПоставщикукнИзменитьСтатусЗаказаПоставщика(Кнопка)
	
	ИзменитьСтатус("Входящая");	
	
КонецПроцедуры

Процедура КоманднаяПанельВозвратыОтПокупателякнИзменитьСтатусВозвратаОтПокупателя(Кнопка)
	
	ИзменитьСтатус("Входящая");	
	
КонецПроцедуры

Процедура кнОбновитьСнапшотыНажатие(Элемент)
	
	ЭлементыФормы.Снапшоты.Значение = НайтиПрочиеЗаявки("snapshot");
	ЭлементыФормы.Снапшоты.СоздатьКолонки();
	
КонецПроцедуры

Процедура кнОбновитьКонвертацияПартийНажатие(Элемент)
	
	ТекстЗапроса = "ВЫБРАТЬ
	|	ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.ДатаЗаписи КАК ДатаЗаписи,
	...
	|	ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Источник КАК Источник,
	|	ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Приемник КАК Приемник,
	|	ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Идентификатор КАК Идентификатор,
	...
	|ГДЕ
	|	ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Приемник = ""AutoConverter""
	|	И ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.ДатаЗаписи МЕЖДУ &НачДата И &КонДата
	|	И ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Идентификатор ПОДОБНО ""convertfgdummy%""
	|	И ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Текст ПОДОБНО ""%"" + &КодДистрибьютора + ""%""";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("НачДата", НачалоПериода);
	Запрос.УстановитьПараметр("КонДата", КонецДня(КонецПериода));
	Запрос.УстановитьПараметр("КодДистрибьютора", СвойстваПартнера.КодДистрибьютора);
	
	ЭлементыФормы.КонвертацияПартий.Значение = Запрос.Выполнить().Выгрузить();
	ЭлементыФормы.КонвертацияПартий.СоздатьКолонки();
	
КонецПроцедуры

Процедура кнОбновитьПечатныеФормыНажатие(Элемент)
	
	ЭлементыФормы.ПечатныеФормы.Значение = НайтиПрочиеЗаявки("print");
	ЭлементыФормы.ПечатныеФормы.СоздатьКолонки();		
	
КонецПроцедуры

Функция НайтиПрочиеЗаявки(Идентификатор)
	...
	|	РегистрСведений.ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций КАК ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций
	|ГДЕ
	|	(ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Приемник = &Партнер
	|			ИЛИ ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Источник = &Партнер)
	|	И ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.ДатаЗаписи МЕЖДУ &НачДата И &КонДата
	|	И ЖурналРегистрацииСостоянийЗаявокНаОформлениеОпераций.Идентификатор ПОДОБНО ""%"" + &Идентификатор + ""%""";
...
Процедура СнапшотыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
		
	Если Колонка.Имя = "Текст" Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьТекстЗаявки(ВыбраннаяСтрока);
	КонецЕсли; 
	
КонецПроцедуры

Процедура ПоказатьТекстЗаявки(ВыбраннаяСтрока)
	
	ТекстДок = Новый ТекстовыйДокумент;
	ТекстДок.УстановитьТекст(ВыбраннаяСтрока.Текст);
	ТекстДок.Показать("Снапшот от " + ВыбраннаяСтрока.ДатаЗаписи);	
	
КонецПроцедуры

Процедура КонвертацияПартийВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
...
Процедура ПечатныеФормыВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
...
Процедура КоманднаяПанельЗаказыПоставщикукнКонвертацияПартий(Кнопка)
	
	СтрокаТабЧасти = ЭлементыФормы.ЗаказыПоставщику.ТекущаяСтрока;
	ПеревестиЦифровыеПартииНаВиртуальнуюВСап(СтрокаТабЧасти.ДокументСсылка);
