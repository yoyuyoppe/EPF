...
Процедура КонсолидацияЗаказовНаВыкуп() Экспорт
	...
	|	ГДЕ
	|		ОТУ.ДатаОтгрузки МЕЖДУ &ДатаНачала И &ДатаОкончания
...
Процедура СоздатьДокументВыкупТовараСХранения(ТЗ_СводныйЗаказ) Экспорт
	...
	ДокВыкупТовараСХранения.Комментарий = СтрШаблон("Выкуп на дату отгрузки: %1", Формат(ДатаНачала, "ДЛФ = D"));
...
Функция ПолучитьИмяФайлаВыкупа(ПутьКФайлу)
	...
	ДатаФормированияФайлаВыкупа = Формат(ТекущаяДата(), "ДФ=""ддММгггг_ЧЧммсс""");
