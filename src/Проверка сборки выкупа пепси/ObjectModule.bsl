...
Функция ПроверитьСборкуВыкупаПепси(ОтборВыкупа, СобранныеМарки) Экспорт 
	...
	Отказ = Ложь;
	// 1. Проверим, что собранная марка приходила из Utrace
	// и принадлежит Пепси
	ПроверитьПринадлежностьМарок(СобранныеМарки, Отказ);
	
	// 2. Сравним план и факт выкупленных и собранных по факту маркировочных партий
	КорректировкаВыкупа = Ложь;
	ПроверитьРазницуМеждуВыкупленнымиИСобраннымиМарками(ОтборВыкупа, СобранныеМарки, КорректировкаВыкупа, Отказ);
	
	// 3. Скорректируем количество в отборе запасов, ПСО, ПТУ
	//, чтобы у нас не было излишек
	Если ЗначениеЗаполнено(КорректировкаВыкупа) Тогда
		ИзменитьКоличествоВЦепочкеВыкупа(КорректировкаВыкупа, Отказ);	
	КонецЕсли; 
...
Процедура ИзменитьКоличествоВЦепочкеВыкупа(КорректировкаВыкупа, Отказ)
		
КонецПроцедуры
 

Процедура ПроверитьРазницуМеждуВыкупленнымиИСобраннымиМарками(ОтборВыкупа, СобранныеМарки, КорректировкаВыкупа, Отказ)
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли; 
	
	// КорректировкаВыкупа - таблица значений, куда запишем товар и количество для корректировки
	
КонецПроцедуры

Процедура ПроверитьПринадлежностьМарок(СобранныеМарки, Отказ)
